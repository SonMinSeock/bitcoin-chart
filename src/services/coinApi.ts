...
export const fetchBitcoinPriceDetail = async (currency: "KRW" | "USD", id: string): Promise<CoinData> => {
  try {
    const response = await fetch(`https://api.coinpaprika.com/v1/tickers/${id}?quotes=${currency}`);
    if (!response.ok) {
      throw new Error(`HTTP error! Status: ${response.status} ${response.statusText}`);
    }
    const data = await response.json();
